package com.davinkim.iteratee

import scala.concurrent.{ ExecutionContext, Future, Promise }
import scala.util.control.NonFatal
import com.davinkim.iteratee.Execution.Implicits.{ defaultExecutionContext => dec }
import com.davinkim.iteratee.internal.{ eagerFuture, executeFuture, executeIteratee, identityFunc, prepared }
...
object Iteratee {
  ...
  /**
    * flatten a [[scala.concurrent.Future]] of [[com.davinkim.iteratee.Iteratee]]] into an Iteratee
    *
  ...
  /**
    * Create an [[com.davinkim.iteratee.Iteratee]] which folds the content of the Input using a given function and an initial state
    *
    * Example:
    * {{{
  ...
  /**
    * Create an [[com.davinkim.iteratee.Iteratee]] which consumes and concatenates all Input chunks
    *
    * Example:
    * {{{
  ...
  trait EofOrElse[E] {
    /**
      * @param otherwise Value if the input is not [[com.davinkim.iteratee.Input.EOF]]
      * @param eofValue Value if the input is [[com.davinkim.iteratee.Input.EOF]]
      * @tparam A Type of `eofValue`
      * @tparam B Type of `otherwise`
      * @return An `Iteratee[E, Either[B, A]]` that consumes one input and produces a `Right(eofValue)` if this input is [[com.davinkim.iteratee.Input.EOF]] otherwise it produces a `Left(otherwise)`
      */
  ...
  /**
    * @return an [[com.davinkim.iteratee.Iteratee]] which just ignores its input
    */
  def ignore[E]: Iteratee[E, Unit] = fold[E, Unit](())((_, _) => ())(dec)

  /**
    * @return an [[com.davinkim.iteratee.Iteratee]] which executes a provided function for every chunk. Returns Done on EOF.
    *
    * Example:
    * {{{
    *   // Get all chunks of input
  ...
  /**
    *
    * @return an [[com.davinkim.iteratee.Iteratee]] which pushes the input into the provided [[com.davinkim.iteratee.Iteratee]], starting over again each time it terminates until an EOF is received, collecting a sequence of results of the different use of the iteratee
    *
...
/**
  ...
  * (see the [[com.davinkim.iteratee.Step]] trait):
  * [[com.davinkim.iteratee.Done]], which means it contains a result and potentially some unconsumed part of the stream;
  * [[com.davinkim.iteratee.Cont]], which means it contains a function to be invoked to generate a new Iteratee from the next piece of input;
  * [[com.davinkim.iteratee.Error]], which means it contains an error message and potentially some unconsumed part of the stream.
  *
  ...
  *
  * Typically an [[com.davinkim.iteratee.Enumerator]] would be used to
  * push data into an Iteratee by invoking the function in the [[com.davinkim.iteratee.Cont]]
  ...
  * The state of an Iteratee (the current [[com.davinkim.iteratee.Step]] may not be available
...
trait Iteratee[E, +A] {
  ...
  /**
    ...
    * if the Iteratee is in the [[com.davinkim.iteratee.Cont]] state.
  ...
  /**
    ...
    */
  protected[com] def foldNoEC[B](folder: Step[E, A] => Future[B]): Future[B] =
    fold(folder)(dec)
  ...
  /**
    ...
    * which can execute the `folder` function immediately.
    */
  protected[com] def pureFoldNoEC[B](folder: Step[E, A] => B): Future[B] =
    pureFold(folder)(dec)
  ...
  /**
    ...
    * which can execute the `folder` function immediately.
    */
  protected[com] def pureFlatFoldNoEC[B, C](folder: Step[E, A] => Iteratee[B, C]): Iteratee[B, C] =
...
private sealed trait StepIteratee[E, A] extends Iteratee[E, A] with Step[E, A] {
  ...
  protected[com] final override def foldNoEC[B](folder: Step[E, A] => Future[B]): Future[B] = {
    folder(immediateUnflatten)
  }

  protected[com] final override def pureFoldNoEC[B](folder: Step[E, A] => B): Future[B] = {
    try Future.successful(folder(immediateUnflatten)) catch { case NonFatal(e) => Future.failed(e) }
  }

  protected[com] final override def pureFlatFoldNoEC[B, C](folder: Step[E, A] => Iteratee[B, C]): Iteratee[B, C] = {
    folder(immediateUnflatten)
  }
...
object Done {
  /**
    * Create an [[com.davinkim.iteratee.Iteratee]] in the “done” state.
    * @param a Result
...
object Cont {
  /**
    * Create an [[com.davinkim.iteratee.Iteratee]] in the “cont” state.
...
object Error {
  /**
    * Create an [[com.davinkim.iteratee.Iteratee]] in the “error” state.
