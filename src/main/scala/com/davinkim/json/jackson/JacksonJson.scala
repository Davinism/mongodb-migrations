package com.davinkim.json.jackson
...
/**
  ...
  *
  *   val mapper = new ObjectMapper().registerModule(MongoMigrationJsonModule)
  *   val jsValue = mapper.readValue("""{"foo":"bar"}""", classOf[JsValue])
  * }}}
  */
object MongoMigrationJsonModule extends SimpleModule("MongoMigrationJson", Version.unknownVersion()) {
  override def setupModule(context: SetupContext) {
    context.addDeserializers(new MongoMigrationDeserializers)
    context.addSerializers(new MongoMigrationSerializers)
  }
}
...
private[jackson] class MongoMigrationDeserializers extends Deserializers.Base {
...
private[jackson] class MongoMigrationSerializers extends Serializers.Base {
...
private[json] object JacksonJson {
  ...
  private val mapper = (new ObjectMapper).registerModule(MongoMigrationJsonModule)
