package com.davinkim.json

import com.davinkim.validation.ValidationError
...
case class JsPath(path: List[PathNode] = List()) {
  ...
  object json {
    /**
      * (____ \ 'key).json.pick[A <: JsValue] is a Reads[A] that:
    ...
    /**
      * (____ \ 'key).json.pick is a Reads[JsValue] that:
      * - picks the given value at the given JsPath (WITHOUT THE PATH) from the input JS
    ...
    /**
      * (____ \ 'key).json.pickBranch[A <: JsValue](readsOfA) is a Reads[JsObject] that:
    ...
    /**
      * (____ \ 'key).json.pickBranch is a Reads[JsObject] that:
      * - copies the given branch (JsPath + relative JsValue) from the input JS at this given JsPath
    ...
    /**
      * (____ \ 'key).put(fixedValue) is a Reads[JsObject] that:
    ...
    /**
      * (____ \ 'key).json.copyFrom(reads) is a Reads[JsObject] that:
    ...
    /**
      * (____ \ 'key).json.update(reads) is the most complex Reads[JsObject] but the most powerful:
    ...
    /**
      * (____ \ 'key).json.prune is Reads[JsObject] that prunes the branch and returns remaining JsValue
      *
      * Example :
      * {{{
      * val js = Json.obj("key1" -> "value1", "key2" -> "value2")
